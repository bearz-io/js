...
import { createHash, getHashes, randomFillSync } from "node:crypto";
...
const { STDERR } = CHANNEL_EXTENDED_DATATYPE;
...
class Client extends EventEmitter {
    ...
    connect(cfg) {
        ...
        this.config.allowAgentFwd = cfg.agentForward === true &&
            this.config.agent !== undefined;
        let authHandler = this.config.authHandler = typeof cfg.authHandler === "function" ||
        ...
        const onConnect = (() => {
            ...
            return () => {
                ...
                wasConnected = true;
                debug && debug("Socket connected2");
                this.emit("connect");
                console.log("pre cryptoInit");
                console.log(cryptoInit);
                cryptoInit.then(() => {
                    console.log("post cryptoInit");
                    proto.start();
                    sock.on("data", (data) => {
                        try {
