...
import { readUInt32BE, writeUInt32BE } from "./utils.js";
...
class ChaChaPolyCipherNative {
    ...
    encrypt(packet) {
        ...
        const polyKey = createCipheriv("chacha20", this._encKeyMain, POLY1305_OUT_COMPUTE)
            .update(POLY1305_ZEROS);

        // Encrypt packet length
        const pktLenEnc = createCipheriv("chacha20", this._encKeyPktLen, POLY1305_OUT_COMPUTE)
        ...
        const payloadEnc = createCipheriv("chacha20", this._encKeyMain, POLY1305_OUT_COMPUTE)
        ...
        mac.set(
            new Uint8Array(POLY1305_WASM_MODULE.HEAPU8.buffer, POLY1305_RESULT_MALLOC, 16),
            0,
        );
        this._onWrite(mac);
...
class ChaChaPolyDecipherNative {
    ...
    decrypt(data, p, dataLen) {
        ...
        while (p < dataLen) {
            ...
            POLY1305_OUT_COMPUTE[0] = 0; // Set counter to 0 (little endian)
            writeUInt32BE(POLY1305_OUT_COMPUTE, this.inSeqno, 12);
            const polyKey = createCipheriv("chacha20", this._decKeyMain, POLY1305_OUT_COMPUTE)
                .update(POLY1305_ZEROS);
            ...
            // Decrypt packet
            POLY1305_OUT_COMPUTE[0] = 1; // Set counter to 1 (little endian)
            const packet = createDecipheriv("chacha20", this._decKeyMain, POLY1305_OUT_COMPUTE)
                .update(this._packet);
        ...
        while (p < dataLen) {
            ...
            if (this._blockPos < this._block.length) {
                ...
                if (this._macETM) {
                    ...
                    this._packet = Buffer.allocUnsafe(this._len);
                    this._packet.set(
                        new Uint8Array(decrypted.buffer, decrypted.byteOffset + 4, this._packetPos),
                        0,
                    );
                }
...
function createCipher(config) {
    ...
    console.log("cipherInfo:", outbound.cipherInfo);
...
function createDecipher(config) {
    ...
    console.log("decipherInfo:", inbound.decipherInfo);
...
export const init = new Promise((resolve) => {
    console.log("cryptoInit called");
    resolve();
});

export { CIPHER_INFO, createCipher, createDecipher, MAC_INFO, NullCipher, NullDecipher };
