...
export function bufferFill(buf, value, start, end) {
    return TypedArrayFill.call(buf, value, start, end);
}

export function doFatalError(protocol, msg, level, reason) {
    let err;
...
export function readString(buffer, start, dest, maxLen) {
    if (typeof dest === "number") {
        maxLen = dest;
        dest = undefined;
    }
...
export function sigSSHToASN1(sig, type) {
    switch (type) {
        case "ssh-dss": {
            if (sig.length > 40) {
                return sig;
...
export function convertSignature(signature, keyType) {
    switch (keyType) {
        case "ssh-dss": {
...
export function sendPacket(proto, packet, bypass) {
