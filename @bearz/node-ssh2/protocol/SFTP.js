...
import { bufferCopy, bufferSlice, makeBufferParser, writeUInt32BE } from "./utils.js";
import { inherits, types } from "node:util";
import { ERR_INVALID_ARG_TYPE, ERR_OUT_OF_RANGE, validateNumber } from "./node-fs-compat.js";
    ...
    */
    ...
    name(reqid, names) {
        ...
        for (let i = 0; i < count; ++i) {
            const name = names[i];
            const filename = !name || !name.filename || typeof name.filename !== "string"
                ? ""
                : name.filename;
            namesLen += 4 + Buffer.byteLength(filename);
            const longname = !name || !name.longname || typeof name.longname !== "string"
                ? ""
                : name.longname;
        ...
        if (extensions["limits@openssh.com"] === "1") {
            return requestLimits(sftp, (err, limits) => {
                if (!err) {
                    ...
                    sftp.maxOpenHandles = limits.maxOpenHandles > 0
                        ? limits.maxOpenHandles
                        : Infinity;
                }
                sftp.emit("ready");
            });
...
export { flagsToString, OPEN_MODE, SFTP, Stats, STATUS_CODE, stringToFlags };
