...
import { bindingAvailable, NullCipher, NullDecipher } from "./crypto.js";
import {
    COMPAT_CHECKS,
    DISCONNECT_REASON,
    eddsaSupported,
...
import MESSAGE_HANDLERS from "./handlers.js";
import {
    bufferCopy,
    bufferFill,
    bufferSlice,
...
import { PacketReader, PacketWriter, ZlibPacketReader, ZlibPacketWriter } from "./zlib.js";
import { readFileSync } from "node:fs";

// const MODULE_VER = ('../../package.json').version;
const __filename = import.meta.url.slice(7);
const __dirname = __filename.slice(0, __filename.lastIndexOf("/") + 1);
const parentDir = __dirname.slice(0, __dirname.slice(0, -1).lastIndexOf("/") + 1);
const _content = readFileSync(parentDir + "/deno.json", { encoding: "utf8" });
const _json = JSON.parse(_content);
const MODULE_VER = _json.version;
...
class Protocol {
    constructor(config) {
        ...
        this._onHandshakeComplete = (...args) => {
            ...
            console.log("firstHandshake", firstHandshake);
    ...
    authPK(username, pubKey, keyAlgo, cbSign) {
        ...
        const payloadLen = (cbSign ? 4 + sesLen : 0) +
...
function sendExtInfo(proto) {
    let serverSigAlgs = "ecdsa-sha2-nistp256,ecdsa-sha2-nistp384,ecdsa-sha2-nistp521" +
