// deno-lint-ignore-file no-unused-vars no-empty
// TODO:
...
import { KexInit } from "./protocol/kex.js";
import { parseKey } from "./protocol/keyParser.js";
import Protocol from "./protocol/Protocol.js";
import { SFTP } from "./protocol/SFTP.js";
import { writeUInt32BE } from "./protocol/utils.js";

import { Channel, MAX_WINDOW, PACKET_SIZE, WINDOW_THRESHOLD, windowAdjust } from "./Channel.js";
...
export class Server extends EventEmitter {
    constructor(cfg, listener) {
        ...
        const cfgAlgos = typeof cfg.algorithms === "object" && cfg.algorithms !== null
            ? cfg.algorithms
            : {};
        ...
        this._srv = new netServer((socket) => {
            ...
            new IncomingClient(socket, hostKeys, ident, offer, debug, this, cfg);
        }).on("error", (err) => {
            this.emit("error", err);
        }).on("listening", () => {
            this.emit("listening");
...
export class IncomingClient extends EventEmitter {
    constructor(socket, hostKeys, ident, offer, debug, server, srvCfg) {
        ...
        const kaIntvl = typeof srvCfg.keepaliveInterval === "number" &&
        ...
        const kaCountMax = typeof srvCfg.keepaliveCountMax === "number" &&
                ...
                CHANNEL_OPEN: (p, info) => {
                    ...
                    switch (info.type) {
                        ...
                        default:
                            ...
                            if (debug) {
                                debug(
                                    "Automatic rejection of unsupported incoming channel open" +
                                        ` type: ${info.type}`,
                                );
                            }
                    }
                    ...
                    if (reason === undefined) {
                        ...
                        if (debug) {
                            debug(
                                "Automatic rejection of unexpected incoming channel open" +
                                    ` for: ${info.type}`,
                            );
                        }
                    }
