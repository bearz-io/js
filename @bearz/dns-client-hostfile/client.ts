import type { DnsOperationParams, DnsRecord, DnsRecordsClient } from "@bearz/dns-client/types";
import {
    abort,
    coerceError,
    fail,
...
export class HostfileDnsRecordsClient
    implements DnsRecordsClient, Record<string | symbol, unknown> {
    #backupDir?: string;
    ...
    async listRecords(zone: string, params?: DnsOperationParams): Promise<Result<DnsRecord[]>> {
        try {
        ...
        return this.listRecords(zone, params).then((res) => {
            return res.map((records) => {
                const matches = records.filter((r) =>
                    r.name !== zone && r.name.endsWith("." + zone)
                );
    ...
    async setRecord(
        ...
        try {
            ...
            if (this.#hostfile === this.#oshostfile && !processElevated()) {
                return fail(
                    new Error(
                        "Hostfile driver requires elevated permissions to modify the hosts file at " +
                            this.#hostfile,
                    ),
                );
            }
