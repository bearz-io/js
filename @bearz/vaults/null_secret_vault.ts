...
import type { GetSecretParams, SecretOperationParams, SecretProperties, SecretRecord, SecretVault, SecretVaultFeatures } from "./types.ts";

/**
 * Represents a secret vault that does not store any secrets.
 */
export class NullSecretVault implements SecretVault {
    /**
     ...
     */
    upateSecretProperties(_name: string, _properties: SecretProperties, _params?: GetSecretParams): Promise<Result<void>> {
        return Promise.resolve(voided());
    }
}